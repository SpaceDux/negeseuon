...
import type { Connection, Topic } from "@/libs/components/Sidebar";
...
export interface ConnectionConfigTabContext {
  ...
  connectionId?: string;
  connectionName?: string;
  connection?: Connection; // full connection object if editing
}
...
interface TabContextValue {
  ...
  openTab: {
    (type: "kafka", context: { connection: Connection; topic: Topic }): void;
  ...
  const openTab = useCallback(
    (
      ...
      if (type === "kafka") {
        ...
        const tabId = configContext.connectionId
          ? `config-${configContext.connectionId}`
          : `config-new-${Date.now()}`;

        setTabs((prevTabs) => {
          // Check if tab already exists (for editing existing connections)
          if (configContext.connectionId) {
            const existingTab = prevTabs.find((tab) => tab.id === tabId);
            if (existingTab) {
              setActiveTabId(tabId);
              return prevTabs;
